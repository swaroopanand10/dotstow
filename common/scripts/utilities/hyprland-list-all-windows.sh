...
# current_window_det="$(echo "$active_window")"

window="$(echo "$state" |
  # gojq -r '.[] | select(.monitor != -1 ) | "\(.address) \(.workspace.name)  \(.class)  \(.title) "' |
  gojq -r '.[] | select(.monitor != -1 ) | "\(.workspace.name)  \(.class)  \(.title)        \(.address)"' |
    sed "s|$current_addr|focused ->|" |
    sort |
    rofi -dmenu -i)"

addr="$(echo "$window" | awk '{print $4}')"
ws="$(echo "$window" | awk '{print $1}')"
