...
this only works with quickmarks and not with bookmarks
make sure to eliminate the ' ' from the start of every line
python importer.py $HOME/scripts/browser/bravebookmarks/bookmarks_8_10_23.html -qB > quickmarks
...
def import_netscape_bookmarks(bookmarks_file, bookmark_types, output_format):
    ...
    output_template = {
        ...
        'quickmark': {
            'bookmark': '{tag[tags]} {tag.string} {tag[href]}',
    ...
    for typ in bookmark_types:
        ...
        for tag in tags:
            # If the <a> tag does not have an attribute 'TAGS', create an empty
            # one to not cause a KeyError in the output_template
            if not 'tags' in tag.attrs:
                tag['tags'] = ""
